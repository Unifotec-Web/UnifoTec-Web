...
export default function ProjectPreview({ full = false }: { full?: boolean }) {
  return <div className="grid gap-5 lg:grid-cols-3">{conceptProjects.slice(0, full ? undefined : 3).map((project) => <article key={project.title} className="group overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm">
    <div className="relative aspect-[4/3] overflow-hidden bg-[#f5f3ed]"><Image src={project.image} alt={project.imageAlt} fill sizes="(max-width: 1023px) 100vw, 33vw" className="object-cover transition-transform duration-500 motion-reduce:transition-none group-hover:scale-[1.03]" /></div>
