...
describe("approved launch content", () => {
  ...
  it("publishes a complete company profile and labels portfolio concepts", () => { expect(companyProfile.values.length).toBeGreaterThanOrEqual(4); expect(conceptProjects).toHaveLength(3); expect(conceptProjects.every((project) => project.category.includes("Concept showcase"))).toBe(true); expect(conceptProjects.every((project) => project.image.startsWith("/images/projects/") && project.image.endsWith(".webp") && project.imageAlt.startsWith("Concept ") && existsSync(`public${project.image}`))).toBe(true); });
